namespace CustomPlantClass
{
    public class CustomBigStar : MonoBehaviour
    {
        ...
        public static GameObject SetStar(GameObject StarPrefab)
        {
            ...
            var go = Instantiate(StarPrefab, Instance.transform);
        ...
        {
            GlobalEvent();
            var zombies = Instance.zombieArray;
            if (zombies != null)
            {
                for (int i = zombies.Count - 1; i >= 0; i--)
                {
                    ...
                    PerZombieEvent(z, DamageType.Normal, damage, fromType);
                }
            }
        ...
        public virtual void PerZombieEvent(Zombie zombie, DamageType theDamageType, int theDamage, PlantType reportType = PlantType.Nothing, bool fix = false)
        {
            zombie.TakeDamage(theDamage, null, theDamageType, reportType, fix);
        }
